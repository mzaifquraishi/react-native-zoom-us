...
public class RNZoomUsModule extends ReactContextBaseJavaModule implements ZoomSDKInitializeListener, MeetingServiceListener, LifecycleEventListener {
    ...
    public ZoomSDKAuthenticationListener authListener = new ZoomSDKAuthenticationListener() {
        /**
    ...
    @ReactMethod
    public void startMeetingWithLogin(
            Promise promise
    ) {
        try {
            meetingPromise = promise;
            ...
            if (zoomSDK.isLoggedIn()) {
                ...
                meetingService.startInstantMeeting(reactContext, options);
            }
        } catch (Exception ex) {
            promise.reject("ERR_UNEXPECTED_EXCEPTION", ex);
        }
    ...
    public void login() {
        int result = ZoomSDK.getInstance().loginWithZoom("<userid>", "<password>");
